        ...
        try:
            ...
            try:
                ...
                for item in itens:
                    item_link = item.find_element(by=By.TAG_NAME, value='a').get_attribute('href')
                    url_list.append(item_link)
                print(f'Peguei a pág. {i} na 3ª tentativa')
            except:
                try:
                    chrome.get(getUrl(i))
                    wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'sc-12rk7z2-0')))
                    ...
                    for item in itens:
                        item_link = item.find_element(by=By.TAG_NAME, value='a').get_attribute('href')
                        url_list.append(item_link)
                        print(f'Peguei a pág. {i} na 4ª tentativa')
                except:
...
header = ['data','nome', 'preco', 'ano', 'km', 'cilindradas', 'tipo', 'cidade', 'bairro', 'cep', 'cod', 'preco_anterior' ,'imagem', 'url']
try:
